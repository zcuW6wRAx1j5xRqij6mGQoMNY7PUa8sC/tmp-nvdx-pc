...
const setProfitRatio = item => {
    // ((最新市场价-买入价)* 交易量)/保证金
    return formatNumber(multiply(divide(setProfit(item), item.margin), 100)).toFixed(2)
}
const { t } = useI18n()
...
const tabChange = id => {
    activeTab.value = id
    initData({
        api: id === 0 ? getSpotOrderApi : getContractOrderApi,
    })
}
    ...
    if (activeTab.value === 0) {
        ...
        return [
            ...
            },
            {
                ...
                cb: data => {
                    return data.trade_status === 'open'
                        ? setProfit(data)
                        : friendlyNumber(data.profit)
                },
                cssCb: data => {
                    return (data.trade_status === 'open' ? setProfit(data) : +data.profit) >= 0
                        ? 'green-color'
                        : 'red-color'
                },
            },
...
<template>
    ...
    <MyTab isShrink :tabList="tabList" :tabActive="activeTab" @tabChange="tabChange" class="pl-4" isFew></MyTab>
    <van-list v-model:loading="loading" :finished="finished" :finished-text="$t('common.noMore')" @load="onLoad"
        ...
        <MyShowInfo v-for="item in showList" :key="item.id" :title="item.name" :infoList="showInfoList"
            ...
            <div class="flex mt-3" v-if="activeTab === 1">
            ...
            <div class="flex mt-3" v-if="activeTab === 0">
                <div class="c-order-tip-box" :class="[item.trade_type === 'limit' ? 'green' : 'red']">
                    {{
                        item.trade_type === 'limit'
            ...
            <template #right>
                <div class="status" v-if="activeTab === 0">
                    {{
