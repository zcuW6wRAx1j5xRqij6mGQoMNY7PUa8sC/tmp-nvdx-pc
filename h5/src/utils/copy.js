...
export default async function copy(text, showTxt) {
    if (Capacitor & Capacitor.isNative) {
        try {
            await Clipboard.writeText({ text: text })
            console.log('Text copied to clipboard (native)')
        } catch (err) {
            console.error('Failed to copy text (native): ', err)
        }
